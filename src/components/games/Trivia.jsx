...
export default function Trivia({ onExit }) {
  ...
  const [selected, setSelected]   = useState(null)       // chosen index or null
  ...
  const advanceQ = useCallback((finalSelected, timeLeft, currentQ, currentScore, currentCorrect) => {
    ...
    // short pause to show result, then advance
    setTimeout(() => {
      setQIndex(prev => {
        const next = prev + 1
  ...
  if (phase === 'select') return (
    <GamePanel title="Trivia" onExit={onExit}>
      <div className="trivia-select">
        ...
        <div className="trivia-select-meta">{QUESTIONS_PER_ROUND} questions · 15 seconds each</div>
  ...
  if (phase === 'results') return (
    <GamePanel title="Trivia" onExit={onExit}>
      <div className="trivia-results">
        ...
        <div className="trivia-grade">{correct >= 9 ? '🏆 Perfect!' : correct >= 7 ? '🥇 Great!' : correct >= 5 ? '👍 Good!' : '📚 Keep studying!'}</div>
        {!saved ? (
          ...
          <div className="trivia-saved">✓ Score saved!</div>
        )}
